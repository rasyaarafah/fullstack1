...
export default function TemplatePreviewPage() {
  ...
  const isApproved = letterData?.status?.toUpperCase() === "APPROVED";
        ...
        {loading ? (
          ...
          >
            <div
              ...
              className={`a4-container relative w-[210mm] min-h-[297mm] bg-white p-12 border border-stone-600 shadow-2xl font-serif text-stone-900 text-xs leading-normal flex flex-col justify-between shrink-0 box-border ${
                !isApproved ? "print:hidden" : ""
              }`}
            >
              {!isApproved && (
                <div className="absolute inset-0 flex items-center justify-center overflow-hidden pointer-events-none z-20">
                  <span className="text-rose-600/25 text-6xl font-black uppercase rotate-[-30deg] whitespace-nowrap select-none">
                    BELUM DISETUJUI — JANGAN DICETAK
                  </span>
                </div>
              )}
              <div>
            ...
            {!isApproved && (
              <div className="hidden print:flex w-full min-h-screen items-center justify-center text-center p-10">
                <p className="text-lg font-serif font-bold text-stone-900">
                  Dokumen ini belum disetujui oleh Admin dan tidak dapat dicetak / diekspor.
                </p>
              </div>
            )}
          </div>
